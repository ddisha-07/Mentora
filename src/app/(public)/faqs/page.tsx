      ...
      <div className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-12">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className="rounded-2xl bg-[#0c0805]/95 border border-orange-950/80 hover:border-orange-500/40 transition-all overflow-hidden"
              >
                <button
                  type="button"
                ...
                >
                  <div className="flex items-start sm:items-center gap-4">
                    <span className="font-mono text-xs font-bold text-orange-400 bg-orange-950/80 border border-orange-900/60 px-2.5 py-1 rounded-md">
                      0{index + 1}
                    </span>
                    <div>
                  ...
                  <div
                    className={`w-7 h-7 rounded-full bg-[#140e08] border border-orange-900/60 flex items-center justify-center text-orange-400 font-mono text-sm shrink-0 transition-transform duration-200 ${
                      isOpen ? 'rotate-180 bg-orange-950/60 text-orange-300' : ''
                    }`}
                  >
                    ↓
                  </div>
                ...
                {isOpen && (
                  <div className="px-6 pb-6 sm:px-7 sm:pb-7 pt-1 border-t border-orange-950/40 text-sm text-zinc-300 leading-relaxed">
                    <p className="bg-[#140e08]/60 p-4 rounded-xl border border-orange-950/60 text-zinc-300">
                      {faq.a}
                    </p>
                  </div>
                )}
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-orange-950/40 via-[#120c08] to-orange-950/40 border border-orange-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-xl">
          <div className="space-y-1.5">
            <h4 className="font-bold text-white text-lg">Still have questions?</h4>
            <p className="text-xs sm:text-sm text-zinc-400">Our engineering and advisory team is ready to help you.</p>
          </div>
          <Link
            href="/contact"
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-400 via-orange-500 to-amber-500 hover:from-amber-300 hover:to-orange-400 text-black font-bold text-xs uppercase tracking-wider font-mono shadow-md whitespace-nowrap text-center transition-all"
          >
            Contact Our Team →
          </Link>
        </div>
