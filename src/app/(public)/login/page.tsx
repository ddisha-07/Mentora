...
function LoginForm({ selectedTrackId }: { selectedTrackId?: string | null }) {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="text-center space-y-2">
        <Link href="/" className="inline-block group mb-1">
          <Image
            ...
            width={160}
            height={90}
            className="h-12 w-auto mx-auto object-contain group-hover:scale-105 transition-transform duration-300"
            priority
          />
        </Link>
        <h1
        ...
        >
          {selectedTrackId ? (
            <span className="text-orange-500 font-semibold">
              Sign in to start your enrolled pathway track
            </span>
          ) : (
      ...
      >
        <div
          className={`rounded-[23px] backdrop-blur-xl p-6 sm:p-7 space-y-5 border transition-colors ${
            isBright
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
            ...
            <button
              ...
              className={`w-full py-3 mt-1 rounded-xl font-bold text-xs font-mono uppercase tracking-wider active:scale-95 transition-all disabled:opacity-50 text-white cursor-pointer ${
                isBright
          ...
          <div
            className={`pt-1 text-center text-xs transition-colors ${
              isBright ? 'text-[#57534E]' : 'text-zinc-400'
            }`}
          >
            New to Mentora?{' '}
...
function LoginContent() {
  ...
  return (
    <div className="min-h-[85vh] py-12 px-4 sm:px-6 lg:px-8 max-w-md mx-auto flex flex-col justify-center">
      <LoginForm selectedTrackId={selectedTrackId} />
    </div>
  );
}
